...
(define (proof-result pf prop-cnf)
  (if (axiom? pf)
      (and (member (axiom-clause pf) (cnf-clauses prop-cnf))
           (axiom-clause pf))
      (let ((cl (proof-result (res-proof-pos pf) prop-cnf))
            (cr (proof-result (res-proof-neg pf) prop-cnf)))
        (and cl cr
             (member (pos (res-var pf)) (clause-lits cl))
             (member (neg (res-var pf)) (clause-lits cr))
             (apply clause (append (remove* (pos (res-var pf)) (clause-lits cl))
                                   (remove* (neg (res-var pf)) (clause-lits cr))))))))
...
;        [(var-eq? (car xs) (car ys)) (cons (car xs) (intersect (cdr xs) (cdr ys)))]
;        [(var-<? (car xs) (car ys)) (intersect (cdr xs) ys)]
;        [else (intersect xs (cdr ys))]))


;;Tutaj jest błąd?
;(define (rc-trivial? rc)
;  (null? (intersect (res-clause-pos rc) (res-clause-neg rc))))

(define (rc-resolve rc1 rc2)
  (let ([y (filter (lambda (x) (member x (res-clause-neg rc2))) (res-clause-pos rc1))]
        [z (filter (lambda (x) (member x (res-clause-pos rc2))) (res-clause-neg rc1))])
    (if (not (null? y))
        (let ([v (car y)])
          (res-clause
           (append (eliminate (res-clause-pos rc1) v) (res-clause-pos rc2))
           (append (res-clause-neg rc1) (eliminate (res-clause-neg rc2) v))
           (res v (res-clause-proof rc1) (res-clause-proof rc2))))
        (if (not (null? z))
            (let ([v (car z)])
              (res-clause
               (append (res-clause-pos rc1) (eliminate (res-clause-pos rc2) v))
               (append (eliminate (res-clause-neg rc1) v) (res-clause-neg rc2))
               (res v (res-clause-proof rc2) (res-clause-proof rc1))))
            #f))))
