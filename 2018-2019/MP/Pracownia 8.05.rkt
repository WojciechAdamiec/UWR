...
;;Zeszyt z algebry ostatnia strona - zad 1

(define append-in-expr2
  (letrec-expr 'append (lambda-expr 'xsys
     (if-expr (null?-expr (car-expr (variable 'xsys)))
              (cdr-expr (variable 'xsys))
              (cons-expr (car-expr (car-expr (variable 'xsys)))
                  (app-expr (variable 'append)
                            (cons-expr (cdr-expr (car-expr (variable 'xsys))) (cdr-expr (variable 'xsys)))))))
     (app-expr (variable 'append)
               (cons-expr (cons-expr (const 1) (cons-expr (const 2) (null-expr)) ) (cons-expr (const 7) (null-expr) )))))

(define (append xsys)
  (if (null? (car xsys))
             (cdr xsys)
             (cons (car (car xsys)) (append (cons (cdr (car xsys)) (cdr xsys))) )))
...
(define (map-in-expr f xs)
            (letrec-expr 'map
                         (lambda-expr 'xs
                                      (if-expr (null?-expr (variable 'xs))
                                               (null-expr)
                                               (cons-expr (app-expr f (car-expr (variable 'xs)))
                                                          (app-expr (variable 'map) (cdr-expr (variable 'xs))))))
                         
            (app-expr (variable 'map) xs)))

;(run (map-in-expr (lambda-expr 'x (op '+ (variable 'x) (const 2137))) (cons-expr (const 1) (cons-expr (const 2) (null-expr)) )))
